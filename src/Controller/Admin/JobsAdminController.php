...
{
    // ✅ LIST (Search + Sort)
    #[Route('', name: 'admin_jobs_list', methods: ['GET'])]
    ...
    // ✅ CREATE
    #[Route('/new', name: 'admin_jobs_new', methods: ['GET','POST'])]
    public function new(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $offre->setCategorieOffre((string)$request->request->get('categorie_offre'));
            $offre->setLieu((string)$request->request->get('lieu'));
            $offre->setStatutOffre((string)($request->request->get('statut_offre') ?: 'OUVERTE'));
            $offre->setDateCreationOffre(new \DateTimeImmutable());
    ...
    // ✅ EDIT
    #[Route('/{id}/edit', name: 'admin_jobs_edit', methods: ['GET','POST'], requirements: ['id' => '\d+'])]
    public function edit(Offrejob $offre, Request $request, EntityManagerInterface $em): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $offre->setCategorieOffre((string)$request->request->get('categorie_offre'));
            $offre->setLieu((string)$request->request->get('lieu'));
            $offre->setStatutOffre((string)($request->request->get('statut_offre') ?: $offre->getStatutOffre()));
    ...
    // ✅ DELETE
    #[Route('/{id}/delete', name: 'admin_jobs_delete', methods: ['POST'], requirements: ['id' => '\d+'])]
    public function delete(Offrejob $offre, EntityManagerInterface $em): Response
    {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
    ...
    #[Route('/{id}/candidatures', name: 'admin_jobs_candidatures')]
...
): Response
{
    ...
    // ✅ ACCEPT / REFUSE
    #[Route('/candidature/{id}/status/{status}', name: 'admin_jobs_candidature_status', methods: ['POST'], requirements: ['id' => '\d+', 'status' => 'ACCEPTEE|REFUSEE'])]
    public function setCandidatureStatus(
        ...
        if (! $this->isCsrfTokenValid('cand_status_' . $cand->getId() . '_' . $status, (string) $request->request->get('_token'))) {
            ...
            return $this->redirectToRoute('admin_jobs_candidatures', ['id' => $cand->getOffre()->getId()]);
        }
